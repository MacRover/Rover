...
#define TRACK_WIDTH 0.6     // 60cm
#define WHEEL_DIAMETER 0.20 // 20cm
...
void calibrateControlMotor(const std_msgs::Int16MultiArray &motorCalibrateMsg);

ros::Subscriber<std_msgs::Int16MultiArray> calibrate_motor("calibrate_motor", &calibrateControlMotor);
...
// std_msgs::Int16MultiArray motorCalibrateData;
...
// ros::Publisher motorCalibratePub("motor_calibrate_data", &motorCalibrateData);
...
PID feedback0(&(E0A.PIDvelocity), &(P0.velCommand), &(P0.setpoint), pid0.P, pid0.I, pid0.D, P_ON_E, DIRECT);
PID feedback1(&(E1A.PIDvelocity), &(P1.velCommand), &(P1.setpoint), pid1.P, pid1.I, pid1.D, P_ON_E, DIRECT);
PID feedback2(&(E2A.PIDvelocity), &(P2.velCommand), &(P2.setpoint), pid2.P, pid2.I, pid2.D, P_ON_E, DIRECT);
PID feedback3(&(E3A.PIDvelocity), &(P3.velCommand), &(P3.setpoint), pid3.P, pid3.I, pid3.D, P_ON_E, DIRECT);
PID feedback4(&(E4A.PIDvelocity), &(P4.velCommand), &(P4.setpoint), pid4.P, pid4.I, pid4.D, P_ON_E, DIRECT);
PID feedback5(&(E5A.PIDvelocity), &(P5.velCommand), &(P5.setpoint), pid5.P, pid5.I, pid5.D, P_ON_E, DIRECT);

char logdebug_buffer[75]; // make sure this is large enough

void calibrateControlMotor(const std_msgs::Int16MultiArray &motorCalibrateMsg)
{
    uint16_t calMotor = motorCalibrateMsg.data[0];
    uint16_t calDelay = motorCalibrateMsg.data[1];
    uint16_t calIncr = motorCalibrateMsg.data[2];
    uint16_t calPulse = 1500;

    nh.loginfo("Starting Calibration...");
    sprintf(logdebug_buffer, "Motor: %d, Delay: %d, Increment: %d", calMotor, calDelay, calIncr);
    nh.loginfo(logdebug_buffer);
    nh.loginfo("-----------------------");
    ...
    while (flag < 3)
    {
        if ((millis() - cur_time) > calDelay)
        {
            if (calPulse >= MAX_PULSE || calPulse <= MIN_PULSE)
            {
                calIncr = -calIncr;
                flag++;
            }

            char angularVelSelected[12];
            dtostrf(encoders[calMotor]->AngularVel, 6, 5, angularVelSelected); // Leave room for too large numbers!

            sprintf(logdebug_buffer, "Mot|Puls|Meas %d, %d, %s", calMotor, calPulse, angularVelSelected);

            nh.loginfo(logdebug_buffer);

            calPulse += calIncr;
            updateMotorSpeed(motors[calMotor], calPulse);

            if (flag >= 2 && calPulse > 1500)
                break;

            cur_time = millis();
        }
    }
    updateMotorSpeed(motors[calMotor], 1500);
}
...
void initializeMotor(Motor *motor, HardwareTimer **tim, PinName pin)
{
    ...
    // CONVERT velCommand to pulseDuration
...
void updateMotorSpeed(Motor *motor, uint16_t desiredPulse)
{
    // CONVERT velCommand to pulseDuration
    // volatile uint16_t pulseDuration = 0;

    (motor->timer)->setCaptureCompare(motor->channel, desiredPulse, MICROSEC_COMPARE_FORMAT);
}
...
{
    E0A.AngularVel = ((volatile double)E0A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;
    E1A.AngularVel = ((volatile double)E1A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;
    E2A.AngularVel = ((volatile double)E2A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;
    E3A.AngularVel = ((volatile double)E3A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;
    E4A.AngularVel = ((volatile double)E4A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;
    E5A.AngularVel = ((volatile double)E5A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * 2 * 3.141592653;

    E0A.PIDvelocity = ((volatile double)E0A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E0A.pulseCount = 0;

    E1A.PIDvelocity = ((volatile double)E1A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E1A.pulseCount = 0;

    E2A.PIDvelocity = ((volatile double)E2A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E2A.pulseCount = 0;

    E3A.PIDvelocity = ((volatile double)E3A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E3A.pulseCount = 0;

    E4A.PIDvelocity = ((volatile double)E4A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E4A.pulseCount = 0;

    E5A.PIDvelocity = ((volatile double)E5A.pulseCount / 1024.0) * (1000000.0 / (volatile double)REFRESH_RATE) * (WHEEL_DIAMETER * 3.141592653); // Rotations per Second * Pi * Diameter
    E5A.pulseCount = 0;
}
