...
PID feedback4(&(E4A.PIDvelocity), &(P4.PIDduration), &(P4.setpoint), 0, 0, 0, P_ON_E, DIRECT);
...
{
    char intbuf[20];
    sprintf(intbuf, "%d", motor->PIDduration);
    nh.loginfo(intbuf);
...
void rolloverCallback(void)
{
    ...
    // E3A.pulseCount = 0;
    // nh.loginfo("setting pid velocity");
    E4A.PIDvelocity = ((volatile double)E4A.pulseCount / 1024.0) * (6000000.0 / (volatile double)REFRESH_RATE);
    // char log_msg[20];
    // char result[8]; // Buffer big enough for 7-character float
    // dtostrf(E4A.pulseCount, 6, 2, result); // Leave room for too large numbers!
    // sprintf(log_msg, "e4aPC:%s", result);
    // nh.loginfo(log_msg);
    E4A.pulseCount = 0;
...
void setup()
{
    ...
    feedback4.SetOutputLimits(-2000, 2000);
...
void loop()
{
    ...
    */
    if ((ctr++) % 50000000 == 0)
    {
        ...
        char PIDduration[8];
        char setpoint[8];
        dtostrf(E4A.PIDvelocity, 6, 2, PIDvelocity);
        dtostrf(P4.PIDduration, 6, 2, PIDduration);
        dtostrf(P4.setpoint, 6, 2, setpoint);
...
//       }
    ...
    dtostrf(computed[0], 6, 2, error); // Leave room for too large numbers!
    dtostrf(computed[0], 6, 2, dInput); // Leave room for too large numbers!
    dtostrf(computed[0], 6, 2, output); // Leave room for too large numbers!
